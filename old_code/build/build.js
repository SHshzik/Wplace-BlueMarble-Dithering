...
const metaContent = fs.readFileSync('src/RedMarble.meta.js', 'utf8');
...
esbuild.build({
  ...
  bundle: true,
  outfile: 'dist/RedMarble.user.css',
  minify: true
});
...
const resultEsbuild = await esbuild.build({
  ...
  outfile: 'dist/RedMarble.user.js', // The file the bundled code is exported to
...
fs.writeFileSync('dist/RedMarble.user.js', resultTerser.code, 'utf8');
...
if (!isGitHub) {
  try {
    importedMapCSS = JSON.parse(fs.readFileSync('dist/RedMarble.user.css.map.json', 'utf8'));
  } catch {
...
const mapCSS = mangleSelectors({
  inputPrefix: 'bm-',
  outputPrefix: 'bm-',
  pathJS: 'dist/RedMarble.user.js',
  pathCSS: 'dist/RedMarble.user.css',
...
if (mapCSS) {
  fs.writeFileSync('dist/RedMarble.user.css.map.json', JSON.stringify(mapCSS, null, 2));
}

// Adds the banner
fs.writeFileSync(
  'dist/RedMarble.user.js',
  metaContent + fs.readFileSync('dist/RedMarble.user.js', 'utf8'),
  'utf8'
);
